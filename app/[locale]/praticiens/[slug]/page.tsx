...
export async function generateMetadata({
  ...
  const rawImage = practitioner.image ?? "/og-default.jpg";
  const ogImage = rawImage.startsWith("http") ? rawImage : `https://cmiev.ch${rawImage}`;
          ...
          <div className="lg:col-span-2">
            <AnimatedSection>
              ...
              <div className="mb-8 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-muted-foreground">
                ...
                {practitioner.email && !["elio-bosani", "severine-schwab", "corinne-dauve"].includes(practitioner.slug) && (
              ...
              <div className="mt-8 space-y-6 text-lg leading-relaxed text-muted-foreground">
                {practitioner.bio[loc].map((p, i) =>
                  p.startsWith("## ") ? (
                    <h3 key={i} className="pt-4 font-heading text-xl font-bold text-foreground sm:text-2xl">
                      {p.slice(3)}
                    </h3>
                  ) : (
                    <p key={i}>{p}</p>
